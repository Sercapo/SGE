...
class GCF(models.Model):
    ...
    alumno_id = fields.Integer(string='Id del alumno', required=True)

    nombre = fields.Char(string='Nombre', required=True)

    apellido = fields.Char(string='Apellido', required=True)

    modulo_id = fields.Many2many('modulos', string='Modulo',index=True)

    centro_id = fields.Many2one('centros', string='Categoria',index=True)